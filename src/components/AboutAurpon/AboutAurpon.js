...
const AboutAurpon = () => {
    ...
    const [aboutAurponData, aurponAboutData] = useState({})
    useEffect(() => {
        fetch('http://localhost:5000/about/639600ed9127bfa3dee3fc0a')
            .then(res => res.json())
            .then(data => aurponAboutData(data))
    }, [])
    const { title, img_url, point_1, point_2, point_3, detail, youtube_link, facebook_link } = aboutAurponData;
        ...
        <div className="row py-5 my-5 ">
            ...
            <div className=" col-md-7 text-start d-flex justify-content-center align-items-center">
                <div>
                    ...
                    <p>{detail}</p>
                    <button className='btn btn-primary f-btn fw-bolder text-light' onClick={() => handelClick(youtube_link)}>You Tube</button>
                    <button className='btn btn-danger ms-3  f-btn fw-bolder text-light' onClick={() => handelClick(facebook_link)}>Facebook</button>
                </div>
